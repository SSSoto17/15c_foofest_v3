...
export const BackButton = () => {
  ...
  return (
    <Button onClick={() => router.back()} className="cursor-pointer body-copy uppercase font-bold py-4 transition-all duration-150 text-aztec-400  hover:text-aztec-300 hover:scale-105 flex gap-2 items-center">
      <MdOutlineArrowBack />
      Back
    </Button>
  );
...
export function ScrollToButton({ scrollFromTop }) {
  ...
  return (
    <button aria-label="Back to Top" className={`fixed m-12 bottom-0 right-0 rounded-full bg-forest-600 drop-shadow-2xl body-copy font-bold p-4 z-30 cursor-pointer hover:scale-105 transition-all duration-150 opacity-0 ${isVisible && "opacity-100"}`} onClick={scrollToTop}>
...
export function LoadMore({ limit }) {
  ...
  return (
    <footer className="grid place-content-center col-span-full">
      <Link href={newLimit} scroll={false} className="button button-secondary button-size-base font-bold uppercase">
        Load More
      </Link>
    </footer>
  );
