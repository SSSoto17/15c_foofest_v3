...
function Hero() {
  return (
    <section className="full-bleed gap-y-4 row-start-1 row-span-4">
      <Image className="max-h-[70svh] full-bleed row-span-full h-svh object-cover row-start-1 md:h-full" src={hero} alt="Image of FooFest participants. Photo by Roberto Rendon on Unsplash." priority></Image>
...
function TicketCard({ type, children, price, variant }) {
  ...
  return (
    <Link href="/session/reservation/flow/checkout" className="contents">
      <article className={`max-w-lg xl:max-w-md flex-1 px-8 py-6 h-full bg-aztec-950 ${variants[variant]} bg-no-repeat hover:scale-101 transition-transform grid gap-y-4`}>
